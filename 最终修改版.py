...
class NotePad(Tk):
    ...
    theme_color = {"Default": "#000000.#FFFFFF", "Olive Green": "#D1E7E0.#5B8340", "Night Mode": "#FFFFFF.#000000"}
    ...
    def create_menu_bar(self):
        ...
        for k in sorted(self.theme_color):
            themes_menu.add_radiobutton(label=k, variable=self.theme_choice, command=self.change_theme)
    ...
    def create_body(self):
        ...
        self.line_number_bar = Text(self, width=2, padx=3, takefocus=0, border=0, background="#F0E68C", state="disable")
        self.line_number_bar.pack(side="left", fill="y")
        
        # 文本编辑区
        # wrap 如何换行， word表示按照单词自动换行
        # undo=True 表示开启撤销功能
        self.context_text = Text(self, wrap="word", undo=True, font="16")
        # 热键绑定
    ...
    def toggle_highlight(self):
        if self.is_heighlight_line.get():
            ...
            # 通过轮询递归的方式进行处理
            self.context_text.after(100, self.toggle_highlight)
        else:
            self.context_text.tag_remove("active_line", 1.0, END)
    ...
    def find_text_dialog(self):
        ...
        max_width, max_height = self.maxsize()
        align_center = "320x100+%d+%d" % ((max_width-320)/2, (max_height-100)/2)
        ...
        Button(search_dialog, text='查找', command=lambda: self.search_result(search_text.get(), ignore_case_value.get(), search_dialog, search_text)).grid(row=0, column=2, sticky='w'+'e', padx=2, pady=2)
    ...
    def search_result(self, key, ignore_case, search_dialog, search_box):
        ...
        search_dialog.title("发现了%d个匹配的" % matches_found)
